        ...
        <div class="col-lg-9">
            ...
            <nav class="navbar" style="background-color: #efefc8; box-shadow: 3px 3px 3px 0px rgba(46,61,73,0.15);">
                <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
                    ...
                    <a class="nav-item nav-link {{($loop->iteration==1?'active':'')}}" id="nav-profile"
                        data-toggle="tab" href="#tab{{$kat->category_id}}" role="tab" aria-controls="nav-home"
                        aria-selected="true">{{$kat->name}}</a>
                    @endforeach
                </div>
            </nav>
            <div class="tab-content py-3 px-3 px-sm-0" id="nav-tabContent">
                @foreach($kategori as $kat1)
                <div class="tab-pane fade show {{($loop->iteration==1?'active':'')}}" id="tab{{$kat1->category_id}}"
                    role="tabpanel" aria-labelledby="nav-home-tab">
                    <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
                        @foreach($subkategori as $subkat)
                        @if($subkat->categories_id==$kat1->category_id)
                        <a class="nav-item nav-link {{($loop->iteration==1?'active':'')}}" id="nav-profile1"
                            data-toggle="tab" href="#tab1{{$subkat->id}}" role="tab" aria-controls="nav-home"
                            aria-selected="true">{{$subkat->name}}</a>
                        @endif
                        @endforeach
                    </div>
                </div>
            ...
            <div class="tab-content py-3 px-3 px-sm-0" id="nav-tabContent">
                @foreach($subkategori as $kat2)
                <div class="tab-pane fade show {{($loop->iteration==1?'active':'')}}" id="tab1{{$kat2->id}}"
                    role="tabpanel" aria-labelledby="nav-home-tab">
                    <div class="row">
                        @foreach($konten as $data)
                        @if($data->subcategory_id==$kat2->id)
